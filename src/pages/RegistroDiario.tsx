...
export default function RegistroDiario() {
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                            ...
                            <select value={quickAssetId} onChange={e => setQuickAssetId(e.target.value)}
                                ...
                                {assets.map(a => <option key={a.id} value={a.id} className="bg-slate-800">
                                    {a.categoria === 'station' ? `[ST] ${a.codigo_patrimonial}` : `[FL] ${a.placa_principal}`}
                                </option>)}
                            </select>
                        </div>
                        <div className="space-y-2">
            ...
            {/* CARDS DE REGISTROS — movil */}
            <div className="md:hidden space-y-3">
                {loading ? (
                    <div className="py-16 text-center text-xs font-black text-slate-500 uppercase tracking-widest">Cargando registros...</div>
                ) : filtered.length === 0 ? (
                    <div className="py-16 flex flex-col items-center">
                        <span className="material-symbols-outlined text-5xl text-slate-800 mb-4">history_toggle_off</span>
                        <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Sin registros para esta fecha</p>
                    </div>
                ) : filtered.map((r: any) => {
                    const asset = assets.find((a: any) => a.id === r.asset_id)
                    const isPendiente = !r.jornada_completa
                    return (
                        <div key={r.id} className={`bg-slate-800/50 rounded-3xl shadow-sm border p-5 flex flex-col gap-4 ${isPendiente ? 'border-amber-500/20' : 'border-slate-700'}`}>
                            {/* Cabecera de la tarjeta */}
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isPendiente ? 'bg-amber-500/10' : 'bg-sky-500/10'}`}>
                                        <span className={`material-symbols-outlined text-[24px] ${isPendiente ? 'text-amber-500' : 'text-sky-400'}`}>local_shipping</span>
                                    </div>
                                    <div>
                                        <p className="text-base font-black text-slate-200 uppercase tracking-tight">{asset?.placa_principal || 'S/P'}</p>
                                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{r.operador_nombre || 'Sin operador'}</p>
                                    </div>
                                </div>
                                {isPendiente ? (
                                    <span className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 text-amber-500 text-[9px] font-black rounded-xl uppercase tracking-widest border border-amber-500/20">
                                        <span className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse"></span>
                                        En curso
                                    </span>
                                ) : (
                                    <span className={`px-3 py-1.5 text-[9px] font-black rounded-xl uppercase tracking-widest border ${r.estado_dia === 'Operativo' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-slate-800 text-slate-500 border-slate-700'}`}>
                                        {r.estado_dia}
                                    </span>
                                )}
                            </div>

                            {/* Datos clave en grid 3 columnas */}
                            ...
                            {/* Acciones con botones grandes para touch */}
                            <div className="flex gap-2">
                                {isPendiente ? (
                                    <button onClick={() => openCierreJornada(r)}
                                        className="flex-1 bg-amber-600 hover:bg-amber-700 text-white text-[11px] font-black uppercase tracking-widest py-4 rounded-2xl transition-all shadow-lg shadow-amber-900/40 flex items-center justify-center gap-2 active:scale-95">
                                        <span className="material-symbols-outlined text-[20px]">stop_circle</span>
                                        Cerrar Jornada
                                    </button>
                                ) : (
                                    <button onClick={() => openEdit(r)}
                                        className="flex-1 bg-slate-800 text-sky-400 text-[11px] font-black uppercase tracking-widest py-4 rounded-2xl transition-all flex items-center justify-center gap-2 active:scale-95 border border-slate-700">
